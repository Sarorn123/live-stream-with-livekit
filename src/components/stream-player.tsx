...
export default function StreamPlayerWrapper({ userId }: Props) {
  const connectionState = useConnectionState();
  const participant = useRemoteParticipant(userId);
  const tracks = useTracks(Object.values(Track.Source)).filter(
    (track) => track.participant.identity === userId
  );

  if (connectionState !== ConnectionState.Connected || !participant) {
    return (
      <div className="grid rounded aspect-video items-center justify-center border bg-secondary text-sm uppercase text-primary">
    ...
    return (
      <>
        <div className="flex aspect-video items-center justify-center border bg-secondary text-sm uppercase text-primary">
...
export const StreamPlayer = ({ participant }: { participant: Participant }) => {
  ...
  return (
    <>
      <div
        className="relative flex aspect-video border bg-secondary "
        ref={playerEl}
      >
        ...
        <StartAudio
          ...
          className="absolute top-0 h-full w-full border bg-secondary  bg-opacity-75 text-primary"
        />
      </div>
      <h1 className="mt-5 text-xl font-semibold">{activeStream?.title}</h1>
      <p className="mt-2 mb-5">{activeStream?.description}</p>
    </>
  );
};
